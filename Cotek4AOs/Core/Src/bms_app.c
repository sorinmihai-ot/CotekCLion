...
// Updated series assumptions:
//   400s (all)   :  6 cells in series
//   500s HYP     : 12 cells in series
//   500s BMZ     : 13 cells in series
//   600s         : 13 cells in series
//
// NOTE on 13s family (BMZ vs 600s):
//   - CAN ID 0x10000010 payload patterns are used as the *primary* discriminator.
//   - GPIO PC4 (read via BSP_is600s_gpio_high()) is used as a *hint* only.
//   - A BMZ signature on 0x10000010 will override any GPIO guess and force BMZ.
//   - A 600s signature on 0x10000010 plus GPIO-high will lock 600s.
...
/*
 * Series assumptions (from your clarification):
 *   400s  :  6s  → "low voltage group"
 *   500HYP: 12s  → "mid voltage group"
 *   500BMZ/600s: 13s ("high-ish", but same series count)
 *
 * Voltage-based reclassification can safely distinguish:
 *   - 400s vs "not 400s"
 *   - 500HYP vs "13s group"
 *
 * BUT it cannot distinguish 500BMZ vs 600s (both 13s), so that remains
 * primarily ID/signature + GPIO based. To avoid fighting the ID logic, we
 * only apply series-based inference when there is no existing type and no lock.
 */
#define CELL_MIN_V              0.80f
#define CELL_MAX_V              5.00f
#define PACK_MIN_VALID_V        5.00f

#define SERIES_400_CELLS        6
#define SERIES_500HYP_CELLS     12
#define SERIES_TOL              1   /* +/-1 cell tolerance */
...
typedef struct {
    ...
    uint8_t  sig600_hits;    /* "600-like" signature hits */
    uint8_t  sigBMZ_hits;    /* "BMZ-like" signature hits */
    uint8_t  saw_bmz_sig;    /* latched if we ever saw BMZ signature */

    /* GPIO hint for 13s group (PC4) */
    uint8_t  gpio_sampled;   /* 0=not yet sampled, 1=sampled */
    uint8_t  gpio_is600;     /* latched GPIO reading: 1 => pin high => prefer 600s */
} BmsFamilyDetect;
...
/* Force family: used when we see a *definitive* discriminator.
 * This is allowed to override any previous lock (e.g. wrongly latched). */
static inline void force_family(BmsTelemetry *b, uint16_t code) {
    s_det.lock = 0U;
    begin_family(b, code);
    s_det.lock = code;
}
...
/* Try reclassifying between 400s / 500HYP / 13s-group using Vpack and Vcell.
 *
 * IMPORTANT:
 *   - Does NOT override an existing family or lock.
 *   - Only used if battery_type_code == 0 and s_det.lock == 0.
 *   - 13s group (BMZ/600) is *not* distinguished by voltage here.
 */
static uint8_t bms_try_reclassify_by_voltage(BmsTelemetry *b) {
    if (s_det.lock != 0u)        return 0U;  /* trust explicit IDs */
    if (b->battery_type_code)    return 0U;  /* type already chosen from IDs */
    ...
    /* 400s ≈ 6s ±1 */
    if (series >= (SERIES_400_CELLS - SERIES_TOL) &&
        series <= (SERIES_400_CELLS + SERIES_TOL)) {
        inferred = TYPE_400S_HYP;
    }
    /* 500HYP ≈ 12s ±1 */
    else if (series >= (SERIES_500HYP_CELLS - SERIES_TOL) &&
             series <= (SERIES_500HYP_CELLS + SERIES_TOL)) {
        inferred = TYPE_500S_HYP;
    }
    /* 13s group (BMZ/600) is ambiguous by voltage alone → do nothing */
    else {
        return 0U;
    }

    if (inferred != 0 && inferred != b->battery_type_code) {
        b->battery_type_code = inferred;
        printf("BMS: inferred by Vpack/Vcell: series=%d (%.2f/%.2f) => %s\r\n",
...
/* ===================== 13s-family GPIO classifier (BMZ vs 600s) ============= */

/*
 * Called when we *know* we are in the 0x1000001x extended family (13s group).
 * - Samples PC4 exactly once via BSP_is600s_gpio_high().
 * - Uses GPIO as a *hint* only: it sets an initial family but does NOT lock.
 * - BMZ/600 CAN signatures (on 0x10000010) can override this later.
 */
static void classify_13s_family_by_gpio(BmsTelemetry *b) {
    /* If we're hard-locked to some non-13s type, do nothing. */
    if (s_det.lock &&
        s_det.lock != TYPE_500S_BMZ &&
        s_det.lock != TYPE_600S) {
        return;
    }

    if (!s_det.gpio_sampled) {
        bool high = BSP_is600s_gpio_high();
        s_det.gpio_sampled = 1U;
        s_det.gpio_is600   = high ? 1U : 0U;

        printf("BMS: 13s ext family classified by GPIO: %s (pin=%u)\r\n",
               s_det.gpio_is600 ? "600s" : "500s BMZ",
               (unsigned)s_det.gpio_is600);
    }

    /* If not yet locked, set an initial family based on GPIO, but don't lock. */
    if (!s_det.lock && b->battery_type_code == 0) {
        uint16_t code = s_det.gpio_is600 ? TYPE_600S : TYPE_500S_BMZ;
        begin_family(b, code);
        /* no s_det.lock here: we want signatures to be able to override */
    }
}
...
static int parse_ext_100000xx(uint32_t id, uint8_t dlc, const uint8_t *d, BmsTelemetry *b) {
    ...
    switch (id) {
        case ID_EXT_10: {
            /* Common V/I fields (shared between BMZ and 600 in your logs): */
            if (dlc >= 4) {
            ...
            /* First, classify the 13s family using GPIO as a hint. */
            classify_13s_family_by_gpio(b);

            /*
             * Now apply CAN signature heuristics on 0x10000010 to
             * definitively distinguish BMZ vs 600s, overriding GPIO
             * if necessary.
             *
             * HEURISTICS from your logs:
             *
             *   - BMZ signature example: d0==0x01, d1==0xC0, d7==0x01
             *   - 600s signature example: d0==0x02, d1==0x25, d7==0x05
             *
             * BMZ signature is treated as *authoritative* and will force BMZ.
             * 600s signature plus GPIO-high can lock 600s.
             */
            if (dlc >= 8) {
                const uint8_t d0 = d[0];
                const uint8_t d1 = d[1];
                const uint8_t d7 = d[7];

                /* BMZ pattern: force BMZ even if GPIO says "600". */
                if (d0 == 0x01 && d1 == 0xC0 && d7 == 0x01) {
                    s_det.saw_bmz_sig = 1U;
                    if (s_det.sigBMZ_hits < 3U) s_det.sigBMZ_hits++;

                    /* BMZ signature is authoritative. */
                    force_family(b, TYPE_500S_BMZ);
                    return 1;
                }

                /* 600-like pattern: only lock 600 if GPIO agrees or GPIO is unknown. */
                if (d0 == 0x02 && d1 == 0x25 && d7 == 0x05) {
                    if (s_det.sig600_hits < 3U) s_det.sig600_hits++;

                    if (!s_det.gpio_sampled || s_det.gpio_is600) {
                        /* If GPIO says 600 (or we haven't sampled yet),
                         * we can treat this as strong evidence for 600s. */
                        if (s_det.sig600_hits >= 2U) {
                            force_family(b, TYPE_600S);
                        } else {
                            /* provisional 600s until we have more hits */
                            choose_family(b, TYPE_600S, false);
                        }
                    } else {
                        /* GPIO explicitly says "BMZ" but signature looks 600ish:
                         * in this case, prefer GPIO; do not override. */
                    }
                    return 1;
                }
            }
        ...
        /*
         * Extended frames carrying SoC, Hi/Lo cell, temps, etc.
         * We treat them as DATA ONLY and deliberately do NOT classify
         * them as "600-only" because logs showed they can appear on BMZ too.
         */
        case ID_EXT_20: { /* typically SoC, etc. */
            if (dlc >= 4) {
                b->soc_percent = d[3];
            }
            return 1;
        ...
        case ID_EXT_100: { /* Hi/Lo cell in 1mV units */
            if (dlc >= 4) {
        ...
        case ID_EXT_110: { /* pack/system temps 0.1C */
            if (dlc >= 4) {
        ...
        case ID_EXT_50:
        case ID_EXT_00: {
            /* Additional status; currently not decoded in detail. */
            return 1;
        }
...
static int parse_400(uint32_t id, uint8_t dlc, const uint8_t *d, BmsTelemetry *b) {
    ...
    if (b->battery_type_code == 0) {
        begin_family(b, TYPE_400S_HYP); /* default 400s */
    }
...
int BMS_ParseFrame(CanFrameEvt const *f, BmsTelemetry *b) {
    ...
    if (parse_400(id, dlc, d, b))            return 1; /* exclusive 400 IDs */
...
static QState Bms_active(BmsAO * const me, QEvt const * const e) {
    ...
    case BMS_TICK_SIG: {
        ...
        /* Late sanity: derive series & reclassify if needed (10 Hz).
         * Only acts when there is no ID-based classification yet. */
        if (me->have_any_data) {
